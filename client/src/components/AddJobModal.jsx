...
const AISearchTab = ({ onSelect }) => {
  ...
  const startListening = () => {
    ...
    rec.onerror = () => { setListening(false); toast.error("Voice input failed — try typing instead"); };
  ...
  const handleSearch = async () => {
    ...
    try {
      const data = await api.post("/jobs/search-descriptions", { title: query.trim(), company: company.trim() });
  ...
  return (
    <div className="space-y-4">
      <div className="bg-brand-50 border border-brand-100 rounded-xl p-3.5">
        <p className="text-xs font-semibold text-brand-800 mb-0.5">AI job description finder</p>
        <p className="text-[11px] text-brand-600">
          Type or speak the job title — Seevv generates three realistic JDs for you to pick from.
        </p>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex gap-2">
          <div className="relative flex-1">
            <input
              ...
              className="w-full px-3 py-2.5 pr-10 text-sm rounded-lg border border-gray-200 hover:border-gray-300 focus:outline-none focus:ring-2 focus:ring-brand-600 focus:border-transparent placeholder:text-gray-400"
            />
            {hasSpeechRecognition() && (
              <button
                type="button"
                onClick={listening ? stopListening : startListening}
                title={listening ? "Stop listening" : "Speak job title"}
                className={`absolute right-2.5 top-1/2 -translate-y-1/2 transition-colors cursor-pointer ${
                  listening ? "text-red-500 animate-pulse" : "text-gray-400 hover:text-brand-600"
                }`}
              >
          ...
          <input
            ...
            className="w-36 px-3 py-2.5 text-sm rounded-lg border border-gray-200 hover:border-gray-300 focus:outline-none focus:ring-2 focus:ring-brand-600 placeholder:text-gray-400"
          />
        </div>
        ...
        {listening && (
          <div className="flex items-center gap-2 text-xs text-red-600 font-medium">
            ...
            Listening… speak your job title
          </div>
        )}
        ...
        <button
          ...
          className="w-full py-2.5 text-xs font-semibold bg-brand-600 text-white rounded-lg hover:bg-brand-700 disabled:opacity-40 cursor-pointer transition-colors"
        >
          {searching ? (
      ...
      {results.length > 0 && (
        <div className="space-y-3">
          <p className="text-xs font-semibold text-gray-700">Pick the description that best matches the role:</p>
          {results.map((r, i) => (
            <button
              key={i}
              type="button"
              onClick={() => onSelect(r.description, r.label)}
              className="w-full text-left p-4 rounded-xl border-2 border-gray-100 hover:border-brand-400 hover:bg-brand-50/50 cursor-pointer transition-all group"
            >
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs font-bold text-gray-800 group-hover:text-brand-700">{r.label}</span>
...
const AddJobModal = ({
  ...
  const [jdMode, setJdMode] = useState("find"); // find | paste | upload | url
  ...
  // ── AI JD selection handler ────────────────────────────────

  const handleAISelect = useCallback(
    async (description, label) => {
      setValue("jobDescription", description, { shouldValidate: true, shouldDirty: true });
      setJdMode("paste");
      toast.success(`"${label}" selected — filling fields…`);
      await smartFillFields(description);
    },
    [setValue, toast, smartFillFields],
  );
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex gap-1 bg-gray-100 rounded-lg p-0.5">
                    <TabBtn active={jdMode === "find"} onClick={() => setJdMode("find")}>
                      Find JD
                    </TabBtn>
                ...
                {/* Find JD tab */}
                {jdMode === "find" && (
                  <AISearchTab onSelect={handleAISelect} />
                )}
